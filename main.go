...
func getCommitShort(ref string) string {
	output, err := exec.Command("git", "rev-parse", "--short", ref).Output()
	if err != nil {
		return "?"
	}
...
func run() error {
	...
	// Run base-branch benchmarks in a detached worktree so the consumer's working
	// tree is never mutated by this action. The worktree shares $GOCACHE and
	// $GOMODCACHE with the HEAD build, so unchanged packages don't re-compile.
	worktreeDir, err := os.MkdirTemp("", "benchmark-base-*")
	if err != nil {
		return fmt.Errorf("failed to create worktree dir: %w", err)
	}
	if _, err := runCommand("git", "worktree", "add", "--detach", worktreeDir, "refs/remotes/base"); err != nil {
		return fmt.Errorf("failed to add worktree for base branch '%s': %w", baseBranch, err)
	}
	defer func() {
		if _, err := runCommand("git", "worktree", "remove", "--force", worktreeDir); err != nil {
			fmt.Println("failed to remove worktree:", err)
		}
	}()

	fmt.Println("Running benchmarks on", baseBranch, "...")
	start = time.Now()
	baseBenchmarks, err := runBenchmarks(config, worktreeDir)
	if err != nil {
		fmt.Println("Failed to run benchmarks on base branch, skipping checks:", err)
		return nil
	}
	fmt.Println("Took", time.Since(start))
	baseCommit := getCommitShort("refs/remotes/base")
